...
class CNN1DResNetExtractor(BaseFeaturesExtractor):
    def __init__(
        ...
        if (
            ...
            raise ValueError(
                "context_size must be 1 and lidar_horizontal_resolution and camera_horizontal_resolution must be 1024 for TemporalResNetExtractor"
            )
        ...
        with torch.no_grad():
            n_flatten = net(
                torch.zeros(
                    [1, 2, context_size, lidar_horizontal_resolution],
                    dtype=torch.float32,
                    device=device,
                )
            ).shape[1]

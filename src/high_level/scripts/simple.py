...
drive_direction = (
    1  # -1 for inverted ESCs or if a small ratio corresponds to forward motion
)
pwm_stop_drive = 7.53
dead_zone_drive = 0.5
...
steering_direction = (
    1  # 1 for min PWM angle to the left, -1 for min PWM angle to the right
)
...
pwm_drive = HardwarePWM(
    pwm_channel=0, hz=50, chip=2
)  # chip 2 on Pi 5 per documentation
...
def set_steering_degree(angle_deg):
    ...
    steering_pwm = steering_pwm_center + steering_direction * (
        steering_pwm_max - steering_pwm_min
    ) * angle_deg / (2 * max_steering_angle_deg)
    ...
    while True:
        for angle in range(len(lidar_table_mm)):
            ...
            if angle > 135 and angle < 225:
                ...
                lidar_table_mm[angle] = lidar.r_distance[540 + (-angle * 4)]
